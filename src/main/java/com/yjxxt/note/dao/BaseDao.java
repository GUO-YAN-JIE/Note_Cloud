    ...
    public static List queryRows(String sql,List<Object> params,Class cls){
        ...
        try {
            ...
            while (resultSet.next()){
                //实例化对象
                Object obj = cls.newInstance();//通过反射创建实例对象
                ...
                for (int i=1;i<=fieldNum;i++){
                    ...
                    Field field = cls.getDeclaredField(columnName);//反射获取属性（设置属性值set,获取属性值get）
                    //拼接set方法，得到字符串
                    String setMethod = "set"+columnName.substring(0,1).toUpperCase()+columnName.substring(1);
                    //通过反射，将set方法字符串反射成类中对应的set方法
                    Method method = cls.getDeclaredMethod(setMethod,field.getType());//反射获取方法
                    //得到查询的每一个字段对应的值
